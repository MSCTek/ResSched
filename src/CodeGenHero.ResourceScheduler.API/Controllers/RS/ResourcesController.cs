...
{
	public partial class ResourcesRSController : RSBaseApiController
	{
			private const string GET_LIST_ROUTE_NAME = "ResourcesRSList";
			private const int maxPageSize = 100;

			private GenericFactory<entRS.Resource, dtoRS.Resource> _factory 
				= new GenericFactory<entRS.Resource, dtoRS.Resource>();

		public ResourcesRSController() : base()
		{
		}

		public ResourcesRSController(ILoggingService log, IRSRepository repository)
		: base(log, repository)
		{
		}

		[HttpDelete]
		[VersionedRoute(template: "Resources/{id}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(System.Guid id)
		{
			try
			{
				...
				var result = await Repo.Delete_ResourceAsync(id);
		...
		[HttpGet]
		[VersionedRoute(template: "Resources", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryable_Resource().AsNoTracking();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entRS.Resource).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpGet]
		[VersionedRoute(template: "Resources/{id}/{numChildLevels:int=0}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(System.Guid id, int numChildLevels)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				var dbItem = await Repo.Get_ResourceAsync(id, numChildLevels);
		...
		[HttpPatch]
		[VersionedRoute(template: "Resources/{id}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(System.Guid id, [FromBody] JsonPatchDocument<dtoRS.Resource> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var dbItem = await Repo.Get_ResourceAsync(id, numChildLevels: 0);
				if (dbItem == null)
				{
					return NotFound();
				}
		...
		[HttpPost]
		[VersionedRoute(template: "Resources", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody] dtoRS.Resource dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		[HttpPut]
		[VersionedRoute(template: "Resources/{id}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(System.Guid id, [FromBody] dtoRS.Resource dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		partial void RunCustomLogicAfterInsert(ref CodeGenHero.ResourceScheduler.Repository.Entities.RS.Resource newDBItem, ref IRepositoryActionResult<entRS.Resource> result);

		partial void RunCustomLogicAfterUpdatePatch(ref CodeGenHero.ResourceScheduler.Repository.Entities.RS.Resource updatedDBItem, ref IRepositoryActionResult<entRS.Resource> result);

		partial void RunCustomLogicAfterUpdatePut(ref CodeGenHero.ResourceScheduler.Repository.Entities.RS.Resource updatedDBItem, ref IRepositoryActionResult<entRS.Resource> result);

		partial void RunCustomLogicOnGetEntityByPK(ref entRS.Resource dbItem, System.Guid id, int numChildLevels);

		partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entRS.Resource> dbItems, ref List<string> filterList);
	}
}